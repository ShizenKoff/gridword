...
// ------------------------------------------------------------
// GRIDWORD – Core Game Script
// Version: 1.2.0 – "Operation GridWord Polish"
// Owner: Carson Design & Prototyping – Interactive (CAR-NOVA.i)
//
// ARCHITECTURE
//  - CONFIG & CONSTANTS
//  - GLOBAL STATE
...
// >>> [NOVA-SECTION: CONFIG & CONSTANTS – START]
// ============================================================
...
// Difficulty → % of letters auto-revealed as given cells
const DIFF_HINTS = {
  easy:   0.55,
  normal: 0.33,
  hard:   0.18,
...
const GW_CONFIG = {
  ...
  // TODO: add more config as needed (colors, fonts, layout thresholds…)


};

// <<< [NOVA-SECTION: CONFIG & CONSTANTS – END]
// #endregion
// ============================================================
...
// ============================================================
// #region GLOBAL STATE
// >>> [NOVA-SECTION: GLOBAL STATE – START]
// ============================================================

/**
 * All mutable runtime state lives here.
 * No random globals scattered around – keep it centralized.
 */
...
// >>> [NOVA-SECTION: DOM REFERENCES – START]
// ============================================================
...
// >>> [NOVA-SECTION: INIT & BOOTSTRAP – START]
// ============================================================
...
window.startGridlocked = async function () {
  ...
  if (isFB) {
    try {
      ...
      // Let FB know we’re ready to start
...
function handleShareClick() {
  console.log('handleShareClick → fired');

  // Use the public GridWord page URL, not the iframe URL
  const url = 'https://www.carson-designs.com/gridword';
  ...
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(url)
      ...
      .catch(err => {
        console.error('Clipboard error, falling back:', err);
        fallbackCopy(url);
      });
    return;
  }
...
// <<< [NOVA-SECTION: INIT & BOOTSTRAP – END]
// #endregion
// ============================================================
...
sub.textContent = picked.category
  ? `Category: ${picked.category} • Mode: ${modeLabel}`
  : `Category: mixed • Mode: ${modeLabel}`;
...
helpBtn.onclick = openHowTo;
  ...
  if (streak > 0) {
    dailyChip.innerHTML = `<span class="icon">🔥</span><span>Streak: ${streak}</span>`;
  } else {
    dailyChip.innerHTML = `<span class="icon">🌓</span><span>Streak: --</span>`;
  }
  ...
  for (let r = 0; r < GRID_SIZE; r++) {
    for (let c = 0; c < GRID_SIZE; c++) {
      ...
      if (hintMask[r][c]) {
        ...
        //  - If top row (r === 0) and NOT top-left (c > 0) → go DOWN
        //  - Otherwise → go RIGHT
...
// >>> [NOVA-SECTION: INPUT HANDLING – START]
// ============================================================
...
// <<< [NOVA-SECTION: INPUT HANDLING – END]
// #endregion
// ============================================================
...
/**
 * Placement engine – attempts to interlock 5 words in a 5x5 grid.
    ...
    for (let c = 0; c < GRID_SIZE; c++) {
      ...
      if (!inp.classList.contains('given') && val !== sol) {
        ...
        if (cellEl) {
          ...
          // Super Nova hint flash ✨ (CSS: .cell.hint-flash)
...
// >>> [NOVA-SECTION: PLATFORM INTEGRATION – START]
// ============================================================
...
/*
  ──────────────────────────────────────────────────────────────
    CAR-NOVA.i // HIDDEN RELIC 01 — “STARFLARE GATE”

    There is a URL encoded below in two layers.
    1) Take the hex bytes and convert to ASCII  → base64 string
    2) Take that base64 string and decode       → final URL
    3) Open the gate.
    ...
    Hint for the worthy:
    • hex → ASCII  → base64 string
    • base64 → ASCII  → ????


    If you solved this, you have officially:
    ▸ Looked under the hood
    ▸ Spoke to the starflare
